...
def plot_wave(f_final_l1b, f_final_l2a, beam_id, shot_number, output, h3_index):
    ...
    plt.text(0.1, 0.9, 'RH98: '+ str(rh98) + ', BEAM: ' + str(beam_id) + ', Quality: ' + str(quality), 
             fontsize=8, ha='left', transform=plt.gca().transAxes)
    plt.text(0.1, 0.85, 'Date: ' + str(new_date), 
             fontsize=8, ha='left', transform=plt.gca().transAxes)
    plt.text(0.1, 0.80, 'Lat: ' + str(lat) + ', Lon: ' + str(lon), 
             fontsize=8, ha='left', transform=plt.gca().transAxes)
    ...
    plt.savefig(output + '/' + str(h3_index) + '_' +str(shot_number) + '.jpg') 
    #plt.show()
    plt.close ()
...
if __name__ == "__main__":
        if len(sys.argv) != 4:
                print("Usage: python shotnumber2wave.py <shot_number> <output_dir> <h3_index>")
                sys.exit(1)
        ...
        print('## plotting...')
        plot_wave(f_final_l1b, f_final_l2a, beam_id, shot_number, output,h3_index)